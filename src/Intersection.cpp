...
/*-------------------------------------------------------------------------------------------
 * FP.6c: addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
 * adds a new vehicle to the queue and returns once the vehicle is allowed to enter.
 * Uses the methods TrafficLight::getCurrentPhase and TrafficLight::waitForGreen to block 
...
/*-------------------------------------------------------------------------------------------
 * FP.6b: simulate() at this position, start the simulation of _trafficLight.
 *-------------------------------------------------------------------------------------------
 */
...
{
   // include this part once light conditions has been solved
   if (_trafficLight.getCurrentPhase() == TrafficLightPhase::green)
       return true;
   else
       return false;
  //return true;   // uncomment to debug without used light
